...
int main()
{
    ...
    // No mods: the page says so and names the folder it looked in.
    {
        ...
        CHECK_EQ(rows[0].label, "gbhook 0.1.0 for ghost 0b89556c");
        CHECK_EQ(rows[0].action, Rows::kInert);
        CHECK_EQ(rows[1].label, "No mods installed");
        CHECK_EQ(rows[2].label, "Looked in X:\\game\\mods");
    }
    ...
    // The list: state word padded to eight, id, version when it fits, action = index; missing roots last.
    {
        ...
        CHECK_EQ(rows.size(), (size_t)1 + mods.size() + 1);
        CHECK_EQ(rows[1].label, "ON      gb.qol 0.1.0");
        CHECK_EQ(rows[1].action, 0);
        CHECK_EQ(rows[2].label, "ON      gb.duelarena");
        CHECK_EQ(rows[3].label, "REFUSED gb.old 2.0");
        CHECK_EQ(rows[3].action, 2);
        CHECK_EQ(rows[4].label, "OFF     gb.off");
        CHECK_EQ(rows[5].label, "FAILED  gb.averyveryveryverylongidenti~");
        CHECK_EQ(rows[5].label.size(), (size_t)39);
        CHECK_EQ(rows[6].label, "Missing root D:\\nope");
        CHECK_EQ(rows[6].action, Rows::kInert);
    }

    // Detail: id, folder, state with the stage, the reason wrapped, content, Back.
    {
        auto rows = ModsPage::Detail(mods[2]);
        CHECK_EQ(rows[0].label, "gb.old 2.0");
        CHECK_EQ(rows[1].label, "Folder Old");
        CHECK_EQ(rows[2].label, "REFUSED");
        CHECK_EQ(rows[3].label, "plugin 'Old.dll': built for ABI 2, this");
        CHECK_EQ(rows[4].label, "gbhook speaks ABI 1 -- rebuild the mod");
    ...
    }
    {
        ...
        CHECK_EQ(rows[2].label, "ON, loads at boot");
        CHECK_EQ(rows[3].label, "Content: 20 files built and mounted");
